...
function serveStatic(app) {
  const distPath = process.env.STATIC_DIR ?? path2.resolve(import.meta.dirname, "..", "dist");
...
${input.description}`);
      const id = createId();
      const ticketNumber = `CF-${(/* @__PURE__ */ new Date()).getUTCFullYear()}-${nanoid3(6).toUpperCase()}`;
...
// server/routers/localAccount.ts
...
import { and as and3, eq as eq4, gt } from "drizzle-orm";
import { nanoid as nanoid4 } from "nanoid";
var scrypt = promisify(scryptCallback);
var SESSION_TTL_MS = 30 * 24 * 60 * 60 * 1e3;
var MAX_LOGIN_ATTEMPTS = 8;
var LOGIN_WINDOW_MS = 15 * 60 * 1e3;
var LOCAL_SESSION_COOKIE = "campusfix_local_session";
var loginAttempts = /* @__PURE__ */ new Map();
var dummyHashPromise = hashPassword("CampusFix-credential-check-only");
var LocalAccountError = class extends Error {
  constructor(reason) {
    super(reason);
    this.reason = reason;
  }
...
function assertAttemptAllowed(request) {
...
function readCookie(request) {
  ...
  try {
    return decodeURIComponent(pair.slice(LOCAL_SESSION_COOKIE.length + 1));
  } catch {
    return void 0;
  }
}
function sessionCookieOptions(request) {
  return { ...getSessionCookieOptions(request), sameSite: "lax", maxAge: SESSION_TTL_MS };
}
async function establishLocalSession(response, request, userId) {
  const db = await getDb();
...
async function getLocalSessionUser(request) {
  const token = readCookie(request);
  if (!token) return null;
  const db = await getDb();
  if (!db) return null;
  const rows = await db.select({ user: users }).from(localAccountSessions).innerJoin(users, eq4(localAccountSessions.userId, users.id)).where(and3(eq4(localAccountSessions.tokenHash, tokenDigest(token)), gt(localAccountSessions.expiresAt, /* @__PURE__ */ new Date()))).limit(1);
  return rows[0]?.user ?? null;
}
async function registerLocalAccount(request, response, input) {
  ...
  try {
    const user = await db.transaction(async (tx) => {
      const openId = `local_${nanoid4(24)}`;
      await tx.insert(users).values({ openId, name: username, loginMethod: "local", lastSignedIn: /* @__PURE__ */ new Date() });
      const created = await tx.select().from(users).where(eq4(users.openId, openId)).limit(1);
      if (!created[0]) throw new LocalAccountError("unavailable");
      await tx.insert(localAccounts).values({ id: nanoid4(24), userId: created[0].id, username, passwordHash });
      return created[0];
    });
...
async function loginLocalAccount(request, response, input) {
  assertAttemptAllowed(request);
  const username = validateLocalUsername(input.username);
  const password = assertPassword(input.password);
  const db = await getDb();
  if (!db) throw new LocalAccountError("unavailable");
  const rows = await db.select({ account: localAccounts, user: users }).from(localAccounts).innerJoin(users, eq4(localAccounts.userId, users.id)).where(eq4(localAccounts.username, username)).limit(1);
  ...
  await db.update(users).set({ lastSignedIn: /* @__PURE__ */ new Date() }).where(eq4(users.id, account.user.id));
  await establishLocalSession(response, request, account.user.id);
  clearAttempts(request);
  return account.user;
}
async function logoutLocalAccount(request, response) {
  const token = readCookie(request);
  const db = await getDb();
  if (token && db) await db.delete(localAccountSessions).where(eq4(localAccountSessions.tokenHash, tokenDigest(token)));
...
async function getLocalProfile(userId) {
  const db = await getDb();
  if (!db) throw new LocalAccountError("unavailable");
  const [user] = await db.select({ id: users.id, name: users.name, email: users.email }).from(users).where(eq4(users.id, userId)).limit(1);
  const [profile] = await db.select().from(userProfiles).where(eq4(userProfiles.userId, userId)).limit(1);
  return { user, profile };
}
async function updateLocalProfile(userId, input) {
  const db = await getDb();
  if (!db) throw new LocalAccountError("unavailable");
  const values = {
  ...
  await db.transaction(async (tx) => {
    await tx.update(users).set({ name: values.name, email: values.email }).where(eq4(users.id, userId));
    await tx.insert(userProfiles).values({ userId, ...values }).onDuplicateKeyUpdate({ set: { ...values, updatedAt: /* @__PURE__ */ new Date() } });
  });
  return getLocalProfile(userId);
}
async function changeLocalPassword(request, response, userId, input) {
  const oldPassword = assertPassword(input.oldPassword);
  const newPassword = assertPassword(input.newPassword);
  const db = await getDb();
  if (!db) throw new LocalAccountError("unavailable");
  const [account] = await db.select().from(localAccounts).where(eq4(localAccounts.userId, userId)).limit(1);
  ...
  await db.transaction(async (tx) => {
    await tx.update(localAccounts).set({ passwordHash, updatedAt: /* @__PURE__ */ new Date() }).where(eq4(localAccounts.userId, userId));
    await tx.delete(localAccountSessions).where(eq4(localAccountSessions.userId, userId));
  });
  await establishLocalSession(response, request, userId);
}

// server/routers/localAccount.ts
var usernameSchema = z3.string().trim().min(3).max(32).regex(/^[A-Za-z0-9][A-Za-z0-9_.-]*$/, "Use letters, numbers, dots, hyphens, or underscores.");
var passwordSchema = z3.string().min(12, "Use at least 12 characters.").max(128, "Password is too long.");
var profileSchema = z3.object({
  name: z3.string().max(120).nullable().optional(),
  email: z3.string().email().max(320).nullable().optional(),
  campusId: z3.string().max(64).nullable().optional(),
  campusRole: z3.enum(["student", "faculty", "it_staff"]).optional(),
  department: z3.string().max(140).nullable().optional(),
  program: z3.string().max(160).nullable().optional(),
  yearOfStudy: z3.string().max(32).nullable().optional()
});
var localProtectedProcedure = publicProcedure.use(({ ctx, next }) => {
  if (!ctx.localUser) throw new TRPCError4({ code: "UNAUTHORIZED", message: "Please sign in to access your CampusFix profile." });
...
function accountError(error) {
  if (error instanceof LocalAccountError && ["invalid_input", "username_taken", "invalid_credentials", "rate_limited", "unavailable"].includes(error.reason)) {
    ...
    throw new TRPCError4({ code, message: messages[reason] });
  }
  throw error;
}
var localAccountRouter = router({
  ...
  register: publicProcedure.input(z3.object({ username: usernameSchema, password: passwordSchema })).mutation(async ({ ctx, input }) => {
    try {
  ...
  login: publicProcedure.input(z3.object({ username: usernameSchema, password: passwordSchema })).mutation(async ({ ctx, input }) => {
    try {
  ...
  profile: localProtectedProcedure.query(async ({ ctx }) => {
    try {
      return await getLocalProfile(ctx.user.id);
    } catch (error) {
      return accountError(error);
    }
  }),
  updateProfile: localProtectedProcedure.input(profileSchema).mutation(async ({ ctx, input }) => {
    try {
      return await updateLocalProfile(ctx.user.id, input);
    } catch (error) {
      return accountError(error);
    }
  }),
  changePassword: localProtectedProcedure.input(z3.object({ oldPassword: passwordSchema, newPassword: passwordSchema })).mutation(async ({ ctx, input }) => {
    try {
      await changeLocalPassword(ctx.req, ctx.res, ctx.user.id, input);
      return { success: true };
    } catch (error) {
      return accountError(error);
    }
...
var automationRouter = router({
  ...
  activate: adminProcedure.input(z4.object({ operationKey: z4.enum(["stale-ticket-escalation", "daily-analytics-summary", "system-health-check"]) })).mutation(async ({ ctx, input }) => {
    const db = await getDb();
    if (!db) throw new Error("CampusFix database unavailable");
    const existing = (await db.select().from(scheduledOperations).where(eq5(scheduledOperations.operationKey, input.operationKey)).limit(1))[0];
    ...
    if (existing) await db.update(scheduledOperations).set({ scheduleCronTaskUid: job.taskUid, lastStatus: "idle", details: "Activated; waiting for the first scheduled run." }).where(eq5(scheduledOperations.id, existing.id));
    else await db.insert(scheduledOperations).values({ id: nanoid5(18), operationKey: input.operationKey, scheduleCronTaskUid: job.taskUid, lastStatus: "idle", details: "Activated; waiting for the first scheduled run." });
...
async function createPlan(message, history) {
  ...
  try {
    return JSON.parse(await fastJsonCompletion(messages));
  } catch {
    const response = await invokeLLM({
      model: "gpt-5-mini",
      messages,
    ...
    return JSON.parse(contentAsText2(response.choices[0]?.message.content));
  }
}
...
async function ensureSession(visitorToken, sessionId, openingMessage) {
  const db = await requireDb();
  if (sessionId) {
    const existing = await db.select().from(publicSupportSessions).where(and4(eq6(publicSupportSessions.id, sessionId), eq6(publicSupportSessions.visitorToken, visitorToken))).limit(1);
    if (existing[0]) return existing[0];
  }
  const session = { id: nanoid6(18), visitorToken, title: openingMessage.slice(0, 180) || "IT support session" };
  await db.insert(publicSupportSessions).values(session);
  return session;
}
async function getSessionHistory(sessionId) {
  const db = await requireDb();
  return db.select({ role: publicSupportMessages.role, content: publicSupportMessages.content }).from(publicSupportMessages).where(eq6(publicSupportMessages.sessionId, sessionId)).orderBy(publicSupportMessages.createdAt);
}
async function saveMessage(params) {
  const db = await requireDb();
  await db.insert(publicSupportMessages).values({ id: nanoid6(18), ...params, citations: params.citations ?? null });
  await db.update(publicSupportSessions).set({ updatedAt: /* @__PURE__ */ new Date() }).where(eq6(publicSupportSessions.id, params.sessionId));
}
  ...
  try {
    ...
    if (!closed) {
      ...
      if (plan.escalationRecommended || plan.stage === "escalate") {
        const db = await requireDb();
        await db.update(publicSupportSessions).set({ status: "escalated" }).where(eq6(publicSupportSessions.id, session.id));
      }
...
async function recordPublicOutcome(req, res) {
  try {
    ...
    const db = await requireDb();
    const session = await db.select().from(publicSupportSessions).where(and4(eq6(publicSupportSessions.id, sessionId), eq6(publicSupportSessions.visitorToken, visitorToken))).limit(1);
    if (!session[0]) return res.status(404).json({ error: "Support session not found." });
    await db.update(publicSupportSessions).set({ status: nextPublicSessionStatusForOutcome(outcome) }).where(eq6(publicSupportSessions.id, sessionId));
...
async function createPublicSupportTicket(req, res) {
  try {
    ...
    const existing = await db.select().from(publicSupportTickets).where(eq6(publicSupportTickets.sessionId, sessionId)).orderBy(desc4(publicSupportTickets.createdAt)).limit(1);
    ...
    const plan = await createPlan(issue, history);
    const ticket = {
      id: nanoid6(18),
      ticketNumber: `IT-${(/* @__PURE__ */ new Date()).getFullYear()}-${nanoid6(6).toUpperCase()}`,
      sessionId,
    ...
    await db.update(publicSupportSessions).set({ status: "escalated" }).where(eq6(publicSupportSessions.id, sessionId));
...
import { and as and5, eq as eq7, lt } from "drizzle-orm";
...
async function escalateStaleTickets() {
  ...
  const stale = await db.select().from(tickets).where(and5(eq7(tickets.status, "open"), eq7(tickets.escalated, false), lt(tickets.updatedAt, cutoff)));
  for (const ticket of stale.filter((ticket2) => isStaleUnescalatedOpenTicket(ticket2, now))) {
    await db.update(tickets).set({ escalated: true }).where(eq7(tickets.id, ticket.id));
    await db.insert(ticketEvents).values({ id: nanoid7(18), ticketId: ticket.id, actorUserId: null, type: "escalated", note: "Scheduled safeguard escalated an unresolved ticket after 72 hours." });
  }
...
async function summarizeOperations() {
  ...
  const [open, active, resolved, recentRuns] = await Promise.all([
    db.select().from(tickets).where(eq7(tickets.status, "open")),
    db.select().from(tickets).where(eq7(tickets.status, "in_progress")),
    db.select().from(tickets).where(eq7(tickets.status, "resolved")),
...
async function reviewServiceSignals() {
  const db = await dbOrThrow();
  const recent = await db.select().from(tickets).where(and5(eq7(tickets.status, "open"), eq7(tickets.escalated, false))).limit(50);
...
async function runCampusFixScheduledOperation(req, res) {
  ...
  try {
    ...
    const db = await dbOrThrow();
    const operation = (await db.select().from(scheduledOperations).where(eq7(scheduledOperations.scheduleCronTaskUid, taskUid)).limit(1))[0];
    ...
    await db.update(scheduledOperations).set({ lastRunAt: /* @__PURE__ */ new Date(), lastStatus: "success", details: detail }).where(eq7(scheduledOperations.id, operation.id));
    return res.json({ ok: true, operation: operation.operationKey, detail });
  } catch (error) {
    const detail = error instanceof Error ? error.message : String(error);
    if (taskUid) {
      const db = await getDb();
      await db?.update(scheduledOperations).set({ lastRunAt: /* @__PURE__ */ new Date(), lastStatus: "failed", details: detail.slice(0, 2e3) }).where(eq7(scheduledOperations.scheduleCronTaskUid, taskUid));
    }
